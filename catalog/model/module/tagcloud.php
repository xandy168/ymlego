    ...
    private function generateTagCloud($data) {
        ...
        if ($resize == true) {
            ...
            foreach ($tags as $record) {
                ...
                $cloud[] = '<a href="' . (HTTP_SERVER . 'index.php?route=product/search&amp;filter_tag=' . $record['tag']) . '&amp;tag=' . $record['tag'] . '" style="font-size: ' . $size . 'px" title="' . $record['qty'] . ' ' .$this->language->get('text_href_title') . ' ' . $record['tag'] . '">' . $record['tag'] . '</a> ';
            }
        } else {
            foreach ($tags as $record) {
